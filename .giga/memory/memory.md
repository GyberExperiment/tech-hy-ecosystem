...
### 📊 TRANSACTION HISTORY CRITICAL FIXES (ЯНВАРЬ 2025):
- **Расширенный поиск**: Увеличен диапазон поиска с 5k до 500k блоков с прогрессивной стратегией
- **Progressive Search**: Поиск по диапазонам (10k → 50k → 200k → 500k блоков) для оптимизации
- **Улучшенное логирование**: Детальные console.log для отладки каждого этапа поиска
- **UI Fix**: Убрана лишняя кнопка "Обновить" из пустого состояния
- **LPLocker Events**: Поиск VGTokensEarned, LPTokensLocked, VGTokensDeposited событий
- **ERC20 Events**: Поиск Transfer/Approval событий для VG, VC, LP токенов
- **Event Limits**: Ограничение результатов (20 LPLocker + 30 ERC20) для производительности
- **RPC LIMITS FIX**: Исправлены диапазоны поиска под лимит 50k блоков RPC endpoint
- **Known Transactions**: Добавлен поиск по известным хэшам транзакций (earnVG, MEV disable)
- **Error Resilience**: Продолжение поиска даже при ошибках в отдельных диапазонах